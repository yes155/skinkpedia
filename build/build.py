...
CONTENT=ROOT/'content'; PUBLIC=ROOT/'public'; DIST=ROOT/'dist'; TEMPLATES=ROOT/'templates'
...
TRUST_DESCRIPTIONS={
 'about.md':'Learn how Skinkpedia researches and publishes evidence-informed guides to skink care, identification, biology and natural history.',
 ...
 'author-farrukh-abdullah.md':'Learn about Farrukh Abdullah, founder and primary research/editorial author of Skinkpedia, and the evidence-led process used across the site.',
...
def description_from(body,title):
    ...
    pieces=[]
    for p in paras:
        if p.startswith(('#','|','- ','* ','1.','2.','3.','>','```','**Page ID')): continue
        t=strip_md(p)
        if not t or t.endswith(':') and len(t)<70: continue
        pieces.append(t)
        joined=' '.join(pieces)
        if len(joined)>=125: return smart_trim(joined)
    if pieces: return smart_trim(' '.join(pieces))
...
def short_label(title):
    if ':' in title: label=title.split(':',1)[0].strip()
    elif '?' in title: label=title.split('?',1)[0].strip()+'?'
    else: label=title.strip()
    if len(label)>42:
        words=label.split(); label=' '.join(words[:7])+('…' if len(words)>7 else '')
    return label

def breadcrumb_chain(pid,pages):
    chain=[]; cur=pid; seen=set()
    while cur and cur not in seen:
        seen.add(cur); p=pages[cur]
...
def base_graph(include_person=True):
    graph=[
    ...
    if include_person:
        graph.append({'@type':'Person','@id':SITE_URL+AUTHOR_URL+'#person','name':AUTHOR_NAME,'url':SITE_URL+AUTHOR_URL})
    return graph

def schema_article(p,desc,breadcrumbs):
    canon=SITE_URL+p['url']; image=SITE_URL+p['hero_image']
    graph=base_graph()
    ...
    if p['page_id']=='SAT-003':
        ...
        graph.append({'@type':'Recipe','@id':canon+'#recipe','name':p['title'],'description':desc,
          ...
          'author':{'@id':SITE_URL+AUTHOR_URL+'#person'},'mainEntityOfPage':{'@id':canon+'#webpage'},
...
def eyebrow_label(p):
    ...
    if p['page_id']=='SAT-001': return 'Word Meaning'
    if p['page_id']=='SAT-002': return 'Language & Usage'
    if p['page_id']=='SAT-003': return 'Scottish Food'
    return {'Foundation':'Skink Basics','Pet Ownership':'Pet Skinks'}.get(section,section)

SINGLE_SPECIES_PAGES={'CR-001','OUT-002','OUT-003','OUT-004','OUT-005','OUT-007','PET-001','PET-002','PET-003','PET-004','PET-005','PET-006','PET-007','PET-008','PET-009','PET-010'}

def toc_from_html(html):
    soup=BeautifulSoup(html,'html.parser'); out=[]
    for h in soup.find_all(['h2','h3']):
        if h.get_text(strip=True).lower()=='references': continue
...
def build():
    ...
    for f in sorted((CONTENT/'articles').glob('*.md')):
        ...
        fm['body_md']=body; fm['description']=description_from(body,fm['title']); pages[fm['page_id']]=fm
    children={k:[] for k in pages}
    for p in pages.values():
        par=p.get('parent_id')
        if par in children: children[par].append(p['page_id'])
    ...
    for f in (CONTENT/'trust').glob('*.md'):
        body=f.read_text(encoding='utf-8'); m=re.search(r'^# (.+)$',body,re.M); title=m.group(1)
        body=re.sub(r'^# .+?\n+','',body,count=1,flags=re.M); html=wrap_tables(render_md(body)); desc=TRUST_DESCRIPTIONS[f.name]; url=trust_routes[f.name]; canon=SITE_URL+url
        breadcrumb=schema_breadcrumb(canon,[{'title':short_label(title),'url':url}])
        if f.name=='author-farrukh-abdullah.md':
            graph=base_graph(include_person=False)+[{'@type':'ProfilePage','@id':canon+'#webpage','url':canon,'name':title,'description':desc,'isPartOf':{'@id':SITE_URL+'/#website'},'mainEntity':{'@id':canon+'#person'},'breadcrumb':{'@id':canon+'#breadcrumb'},'inLanguage':'en'},
                                {'@type':'Person','@id':canon+'#person','name':'Farrukh Abdullah','url':canon,'jobTitle':'Founder, publisher and primary research/editorial author','worksFor':{'@id':SITE_URL+'/#organization'}},breadcrumb]
        else:
            graph=base_graph()+[{'@type':'WebPage','@id':canon+'#webpage','name':title,'url':canon,'description':desc,'isPartOf':{'@id':SITE_URL+'/#website'},'breadcrumb':{'@id':canon+'#breadcrumb'},'inLanguage':'en'},breadcrumb]
        schema=json.dumps({'@context':'https://schema.org','@graph':graph},ensure_ascii=False)
        rendered=trust_t.render(**render_base_kwargs(f'{title} | Skinkpedia',desc,canon,'website',SITE_URL+DEFAULT_OG_IMAGE,'Representative skinks from Skinkpedia',schema),title=title,body_html=html)
        op=out_path(url); op.parent.mkdir(parents=True,exist_ok=True); op.write_text(rendered,encoding='utf-8')

    popular=[]
    for pid,desc in [('BT-001','Large terrestrial skinks with species- and locality-dependent care requirements.'),('PET-001','A colorful burrowing species for keepers who value natural behavior.'),('CR-001','A secretive tropical skink requiring careful humidity and minimal handling.')]:
        p=pages[pid]; popular.append({'title':p['title'].split(':')[0],'url':p['url'],'image':p['hero_image'],'alt':p['hero_alt'],'description':desc})
    ...
    home_graph=base_graph()+[{'@type':'WebPage','@id':SITE_URL+'/#webpage','url':SITE_URL+'/','name':'Skinkpedia | Skink Care, Species Guides & Natural History','description':home_desc,'isPartOf':{'@id':SITE_URL+'/#website'},'about':{'@type':'Taxon','name':'Scincidae','alternateName':'Skinks'},'inLanguage':'en'}]
    home_schema=json.dumps({'@context':'https://schema.org','@graph':home_graph},ensure_ascii=False)
    home=env.get_template('home.html').render(**render_base_kwargs('Skinkpedia | Skink Care, Species Guides & Natural History',home_desc,SITE_URL+'/','website',SITE_URL+DEFAULT_OG_IMAGE,'Representative skinks showing the diversity of the Scincidae family',home_schema),popular=popular)
    (DIST/'index.html').write_text(home,encoding='utf-8')

    (DIST/'404.html').write_text(f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Page not found | Skinkpedia</title><meta name="robots" content="noindex,follow"><link rel="stylesheet" href="/assets/css/site.css"></head><body><main class="container trust-page"><h1>Page not found</h1><p>The page may have moved. Start from <a href="/">Skinkpedia</a>.</p></main></body></html>''',encoding='utf-8')
